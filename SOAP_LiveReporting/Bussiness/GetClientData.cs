...
namespace SOAP_LiveReporting.Bussiness
{
    public class GetClientData
    {
        public List<DataClass.ClientInfo.GetClinetInfomation> GetUserInformation(int UserID)
        {
            ...
            DataTable dt = SqlHelper.GetDataUsingQuery("select top 1 ClientIDP,FirstName,LastName,EmailID,MobileNo,Password,EntryDate,dateofbirth,CityName,AreaOfCity,StateName,Gender from tblClient where  ClientIDP=" + UserID + "");
            foreach (DataRow row in dt.Rows)
            {
                ListUser.Add(new DataClass.ClientInfo.GetClinetInfomation(
                    row["FirstName"].ToString(), 
                    row["LastName"].ToString(), 
                    row["EmailID"].ToString(), 
                    row["MobileNo"].ToString(), 
                    row["EntryDate"].ToString(), 
                    int.Parse(row["ClientIDP"].ToString()), 
                    row["Password"].ToString(), 
                    DateTime.Parse(row["dateofbirth"].ToString()).ToString("dd-MM-yyyy"),
                    row["CityName"].ToString(),
                    row["AreaOfCity"].ToString(),
                    row["StateName"].ToString(),
                    row["Gender"].ToString()
                    ));
            }
            return ListUser;
